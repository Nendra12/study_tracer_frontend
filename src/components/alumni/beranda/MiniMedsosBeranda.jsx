...
// PERBAIKAN: Komponen PostImages agar menangani path API dengan benar menggunakan getImageUrl
function PostImages({ images }) {
  ...
  // Helper untuk mencari path dari berbagai kemungkinan response JSON (url, image_path, dll)
  const getSrc = (img) => {
    if (!img) return '';
...
function StartPostModal({ open, avatarUrl, displayName, onClose, onSubmit, submitting }) {
  ...
  const handleSubmit = async () => {
    if (!content.trim() && images.length === 0) return; // Bisa post gambar aja
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <select value={visibility} onChange={(e) => setVisibility(e.target.value)}
                  className="text-xs font-bold text-slate-500 bg-transparent border-none outline-none cursor-pointer mt-0.5">
            ...
            return (
              <article key={postId} className="w-full rounded-md border border-slate-100 bg-white shadow-sm overflow-hidden">
                <div className="p-5 md:p-6 w-full">
                  ...
                  {/* PEMANGGILAN KOMPONEN GAMBAR DI SINI */}
