    ...
    <footer className="bg-white border-t border-slate-200 py-8 mt-auto relative z-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
        <p className="text-slate-500 text-xs sm:text-sm text-center md:text-left font-medium">
          © {currentYear} Alumni Tracer {theme?.namaSekolah ? `— ${theme.namaSekolah}` : ''}. Hak cipta dilindungi undang-undang
        </p>
        
        <div className="flex flex-wrap justify-center gap-4 sm:gap-6">
          <button 
            onClick={() => openModal('privasi')}
            className="text-slate-500 hover:text-primary text-xs sm:text-sm font-bold transition-colors cursor-pointer"
          >
            Kebijakan Privasi
          </button>
          <button 
