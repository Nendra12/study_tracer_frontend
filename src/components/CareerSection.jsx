...
export default function CareerSection({ jobList }) {
  const jobs = (jobList && jobList.length > 0)
    ? jobList.slice(0, 4).map(j => ({
        role: j.judul || j.title || '-',
        type: j.tipe_pekerjaan || j.type || 'Full-time',
        loc: j.perusahaan?.kota?.nama || j.lokasi || j.location || '-',
        comp: j.perusahaan?.nama || j.company || '-',
        desc: j.deskripsi || j.description || '-',
        banner: (j.foto || j.banner) ? getImageUrl(j.foto || j.banner) : 'https://i.pinimg.com/736x/13/40/11/1340118d98bb8e13d0fc55fa303a13ab.jpg',
      }))
    : fallbackJobs;
