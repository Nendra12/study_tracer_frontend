...
const htmlContentStyles = `
  ...
  .html-content ul, .html-content ol { margin-left: 1.5rem; margin-bottom: 0.5rem; }
  .html-content ul { list-style-type: disc; }
  .html-content ol { list-style-type: decimal; }
  .html-content li { margin-bottom: 0.25rem; }
  .html-content a { color: #3D5A5C; text-decoration: underline; }
  .html-content code { background-color: #f1f5f9; padding: 0.125rem 0.25rem; border-radius: 0.25rem; font-family: monospace; font-size: 0.875em; }
  .html-content pre { background-color: #f1f5f9; padding: 0.75rem; border-radius: 0.5rem; overflow-x: auto; margin-bottom: 0.5rem; }
  .html-content blockquote { border-left: 4px solid #cbd5e1; padding-left: 1rem; font-style: italic; color: #64748b; }
  .html-content h1, .html-content h2, .html-content h3, .html-content h4 { font-weight: 700; margin-top: 0.75rem; margin-bottom: 0.5rem; }
  ...
  .html-content br { display: block; content: ""; margin-top: 0.25rem; }
`;
...
export default function LihatJawabanDetail() {
  ...
  const handleExportExcel = () => {
    ...
    const excelData = pertanyaan.map((item, index) => {
      let jawaban = '-';
      if (item.jawaban) {
        if (item.jawaban.opsi_dipilih) { jawaban = stripHtml(item.jawaban.opsi_dipilih.opsi); } 
        else if (item.jawaban.jawaban_text) { jawaban = stripHtml(item.jawaban.jawaban_text); }
      }
      return {
        'No': index + 1,
    ...
    ws['!cols'] = [{ wch: 5 }, { wch: 60 }, { wch: 50 }, { wch: 15 }];
  ...
  if (loading) {
    return <JawabanDetailSkeleton />;
  }

  if (error) {
    return (
      <div className="p-4 md:p-8 bg-[#F8FAFC] min-h-screen font-sans text-primary">
        <button onClick={() => navigate(-1)} className="cursor-pointer flex items-center gap-2 text-primary/80 hover:text-primary transition-colors text-sm font-bold group mb-6">
  ...
  return (
    <div className="space-y-6 max-w-full overflow-hidden p-1 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <button onClick={() => navigate(-1)} className="cursor-pointer flex items-center gap-2 text-primary/80 hover:text-primary transition-colors text-sm font-bold group">
          <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
          Kembali ke Daftar
        </button>

        <div className="flex gap-3">
          <button onClick={handleExportPDF} className="cursor-pointer flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-white border border-slate-200 rounded-xl text-primary/80 text-sm font-bold shadow-sm hover:bg-slate-50 hover:border-slate-300 transition-all">
            <FileDown size={18} />
            <span className="hidden sm:inline">PDF</span>
          </button>
          <button onClick={handleExportExcel} className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:opacity-90 shadow-md shadow-primary/20 transition-all active:scale-95">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-8">
            ...
            <div className="pt-12 pb-6 px-6 text-center">
              ...
              {/* Lencana Status Profile (Dibuat Solid) */}
              <div className="mt-4 flex justify-center">
                <span className="px-4 py-1.5 bg-primary text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-md">
            ...
            <div className="px-6 pb-6 border-t border-slate-100 pt-4">
              <div className="bg-slate-50 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold text-primary/80 uppercase tracking-wider">Progress</span>
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                {/* Lencana Status Kuesioner Atas */}
                <div className="mb-3 inline-block">
                  <span className="px-3 py-1 bg-primary/10 text-primary border border-primary/20 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm">
                    Status: {kuesioner.status_karir || 'Selesai'}
                  </span>
                </div>
                
                {/* Judul Kuesioner */}
                <div 
                  className="text-2xl font-black text-primary html-content mb-4" 
                  dangerouslySetInnerHTML={createMarkup(kuesioner.judul)} 
                />
                
                {/* PERBAIKAN: Deskripsi Kuesioner (Di bawah judul) dibuat SANGAT JELAS */}
                <div 
                  className="text-sm font-medium text-slate-700 bg-slate-50 p-4 rounded-xl border border-slate-100 html-content leading-relaxed" 
                  dangerouslySetInnerHTML={createMarkup(kuesioner.deskripsi || 'Detail respon kuesioner alumni.')} 
                />
                
                {/* Informasi Tambahan */}
                <div className="flex items-center gap-3 mt-5 text-xs font-bold text-slate-500">
                  <div className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 border border-slate-100 rounded-lg">
              ...
              return (
                <div
                  ...
                  className={`bg-white p-6 rounded-2xl border shadow-sm transition-all hover:shadow-md ${
                    jawaban ? 'border-slate-200' : 'border-orange-200 bg-orange-50/30'
                  }`}
                >
                  {/* Pertanyaan */}
                  <div className="flex gap-4 mb-5">
                    <span className={`shrink-0 w-7 h-7 flex items-center justify-center rounded-full text-xs font-black border ${
                      jawaban ? 'bg-primary/10 text-primary border-primary/20' : 'bg-orange-100 text-orange-600 border-orange-200'
                    }`}>
                      {index + 1}
                    </span>
                    <div className="flex-1">
                    ...
                    {hasOpsi ? (
                      ...
                      <div className="grid grid-cols-1 gap-2.5">
                        {item.opsi_jawaban.map((opsi) => {
                          ...
                          return (
                            <div
                              key={opsi.id_opsi}
                              /* Opsi yang tidak terpilih sekarang bg-primary/10 agar telihat ada kotaknya */
                              className={`relative flex items-center gap-3 px-4 py-3 rounded-xl border text-sm transition-all ${
                                isSelected
                                  ? "bg-primary text-white border-primary shadow-md"
                                  : "bg-primary/10 border-primary/30 text-slate-700"
                              }`}
                            >
                              {/* Custom Radio Button */}
                              <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 transition-colors ${
                                isSelected ? "border-white bg-white" : "border-primary/40 bg-white"
                              }`}>
                                {isSelected && <div className="w-1.5 h-1.5 bg-primary rounded-full" />}
                              </div>

                              <span
                                className={`font-medium html-content ${isSelected ? "font-bold text-white" : ""}`}
                      ...
                      jawaban?.jawaban_text ? (
                        <div className="relative">
                          ...
                          <div
                            className="w-full p-4 pl-10 bg-primary/5 border border-primary/20 rounded-xl text-slate-700 font-medium text-sm leading-relaxed html-content"
          ...
          <div className="flex justify-end pt-4">
            <button
              onClick={() => navigate(-1)}
              className="cursor-pointer px-6 py-2.5 bg-white border border-slate-300 text-slate-500 text-sm font-bold rounded-xl hover:bg-slate-50 hover:text-primary transition-all"
            >
              Tutup Halaman
            </button>
          </div>
...
const InfoItem = ({ icon, label, value }) => (
  <div className="flex items-start gap-3">
    ...
    <div>
      ...
      <p className="text-sm mt-0.5 font-bold text-primary leading-tight">{value}</p>
    </div>
  </div>
);