...
export default function Beranda() {
  ...
  return (
    <div className="w-full mt-13">
      ...
      <section className="relative pt-12 pb-32 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/backgroundAlumni.jpg')" }}>
            ...
            <div className="flex flex-col gap-12">
                  ...
                  {kuesionerPending.length > 0 && (isVerified ? !hasCompletedKuesioner : true) && (
                    <motion.div key="kuesioner-alert" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                      <div className="bg-white rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center gap-6 border border-blue-100 shadow-sm relative overflow-hidden">
                        ...
                        <button onClick={() => { if (kuesionerPending[0]?.id) navigate(`/kuesioner/${kuesionerPending[0].id}`); }} className="w-full md:w-auto bg-primary text-white px-6 py-3 rounded-xl text-sm font-bold hover:bg-opacity-90 transition-all cursor-pointer shadow-lg shadow-primary/20">
                          ISI SEKARANG
                        </button>
                      </div>
                    </motion.div>
              ...
              <section>
                <div className="flex justify-between items-end mb-8">
                  <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">Jejaring Alumni</h2>
                    <p className="text-slate-500 text-sm font-medium mt-1">Kenali rekan sejawat dan bangun koneksi profesional</p>
                  </div>
                  <button onClick={() => { if (canAccessAll) navigate('/alumni/daftar-alumni'); }} className={`group flex items-center gap-2 text-xs font-bold px-5 py-2.5 rounded-full border transition-all ${canAccessAll ? 'bg-white text-primary border-slate-200 hover:border-primary cursor-pointer' : 'text-slate-300 border-slate-100 cursor-not-allowed'}`}>
